...
void sha_stream(SHA_INFO *sha_info, char *fin, int len)
{
    int offset = 0;
    int chunk_size;
    ...
    while (offset < len) {
        chunk_size = (len - offset) > BLOCK_SIZE ? BLOCK_SIZE : (len - offset);

        memcpy(data, fin + offset, chunk_size);
...
{
    eapp_print("%08lx %08lx %08lx %08lx %08lx\n",
